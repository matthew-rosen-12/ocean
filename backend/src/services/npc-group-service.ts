...
export function checkAndHandleCapturedNPCGroupCollisions(room: string): void {
  try {
    // Get all necessary data for collision detection
    const allPaths = Array.from(getpathsfromMemory(room).values());
    const allNPCGroups = getNPCGroupsfromMemory(room);
    ...
    const capturedNPCGroups = Array.from(allNPCGroups.values()).filter(
      npcGroup => npcGroup.phase === NPCPhase.CAPTURED && npcGroup.captorId
    );
...
function calculateNPCGroupScale(numFileNames: number): number {
...
function calculateNPCGroupPosition(captor: any, capturedNPCGroup: NPCGroup): { x: number; y: number; z: number } | null {
  // Get captor's animal scale (this is animalWidth in frontend)
  const animalScale = ANIMAL_SCALES[captor.animal as keyof typeof ANIMAL_SCALES] || 1.0;
  const animalWidth = animalScale;
  
  // Calculate NPC group scale using exact same logic as frontend
  const numFileNames = capturedNPCGroup.fileNames.length;
  if (numFileNames === 0) return null;
  ...
  if (captor.direction) {
    ...
    const length = Math.sqrt(
      captor.direction.x * captor.direction.x + captor.direction.y * captor.direction.y
    );
    if (length > 0.001) {
...
function handleCapturedNPCGroupReflection(
  room: string,
  thrownPathData: pathData,
  thrownNPCGroup: NPCGroup,
  capturedNPCGroup: NPCGroup,
  currentPathPosition: { x: number; y: number; z: number },
  capturedGroupPosition: { x: number; y: number; z: number }
): void {
  ...
  emitToRoom(room, "path-update", { pathData: reflectionPathData });
  ...
  for (let i = 0; i < emissionCount; i++) {
    if (i >= capturedNPCGroup.fileNames.length) break;
  ...
  if (emittedNPCs.length > 0) {
    ...
    emittedNPCs.forEach((emittedNPC) => {
      ...
      emitToRoom(room, "path-update", { pathData: paths.get(emittedNPC.id)! });
      emitToRoom(room, "npc-group-update", { npcGroup: emittedNPC });
    });
    
    // Update or remove the original captured group
    if (restOfNPCsGroup) {
      allNPCGroups.setByNpcGroupId(restOfNPCsGroup.id, restOfNPCsGroup);
      emitToRoom(room, "npc-group-update", { npcGroup: restOfNPCsGroup });
    } else {
      // If no NPCs remain, delete the original group
      allNPCGroups.deleteByNpcGroupId(capturedNPCGroup.id);
      emitToRoom(room, "npc-group-update", { npcGroup: new NPCGroup({ ...capturedNPCGroup, fileNames: [] }) });
    }
