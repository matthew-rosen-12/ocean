...
exports.checkAndHandleCapturedNPCGroupCollisions = checkAndHandleCapturedNPCGroupCollisions;
...
// Check for collisions between captured NPC groups and path NPCs (thrown/returning)
function checkAndHandleCapturedNPCGroupCollisions(room) {
    try {
        // Get all necessary data for collision detection
        const allPaths = Array.from((0, paths_1.getpathsfromMemory)(room).values());
        const allNPCGroups = (0, npc_groups_1.getNPCGroupsfromMemory)(room);
        const allUsers = (0, users_1.getAllUsersInRoom)(room);
        // Get thrown/returning path NPCs
        const thrownPaths = allPaths.filter(path => path.pathPhase === types_1.PathPhase.THROWN || path.pathPhase === types_1.PathPhase.RETURNING);
        // Get captured NPC groups (captured NPCs with captorId)
        const capturedNPCGroups = Array.from(allNPCGroups.values()).filter(npcGroup => npcGroup.phase === types_1.NPCPhase.CAPTURED && npcGroup.captorId);
        for (const thrownPath of thrownPaths) {
            const pathPosition = calculatePathPosition(thrownPath, Date.now());
            const pathNPCGroup = allNPCGroups.getByNpcGroupId(thrownPath.npcGroupId);
            if (!pathNPCGroup)
                continue;
            for (const capturedNPCGroup of capturedNPCGroups) {
                // Skip if it's the same captor (don't collide with your own captured NPCs)
                if (capturedNPCGroup.captorId === pathNPCGroup.captorId) {
                    continue;
                }
                // Calculate captured NPC group position based on captor's position
                const captor = Array.from(allUsers.values()).find(user => user.id === capturedNPCGroup.captorId);
                if (!captor)
                    continue;
                const capturedGroupPosition = calculateNPCGroupPosition(captor, capturedNPCGroup);
                if (!capturedGroupPosition)
                    continue;
                // Use exact same collision detection as thrown NPC vs thrown NPC collisions
                const collided = detectCollision(pathPosition, capturedGroupPosition, types_1.NPC_WIDTH, types_1.NPC_HEIGHT, types_1.NPC_WIDTH, types_1.NPC_HEIGHT);
                if (collided && capturedNPCGroup.fileNames.length > 0) {
                    handleCapturedNPCGroupReflection(room, thrownPath, pathNPCGroup, capturedNPCGroup, pathPosition, capturedGroupPosition);
                    return;
                }
            }
        }
        ...
        console.error("Error checking captured NPC group collisions:", error);
    }
}
// Calculate NPC group scale using exact same logic as frontend
function calculateNPCGroupScale(numFileNames) {
    if (numFileNames === 0)
        return 0;
    // Frontend: calculateNPCGroupScale = baseScale * calculateNPCGroupProportion
    // calculateNPCGroupProportion = 1 + Math.log(numFileNames) / Math.log(4)
    const baseScale = 3;
    const proportion = 1 + Math.log(numFileNames) / Math.log(4);
    return baseScale * proportion;
}
// Calculate captured NPC group position based on captor's position (server-side version)
function calculateNPCGroupPosition(captor, capturedNPCGroup) {
    // Get captor's animal scale (this is animalWidth in frontend)
    const animalScale = types_2.ANIMAL_SCALES[captor.animal] || 1.0;
    const animalWidth = animalScale;
    // Calculate NPC group scale using exact same logic as frontend
    const numFileNames = capturedNPCGroup.fileNames.length;
    if (numFileNames === 0)
        return null;
    const npcScale = calculateNPCGroupScale(numFileNames);
    // Frontend: calculateNPCGroupPosition logic
    ...
    if (captor.direction) {
        // Normalize direction
        const length = Math.sqrt(captor.direction.x * captor.direction.x + captor.direction.y * captor.direction.y);
        if (length > 0.001) {
            directionX = -captor.direction.x / length; // Opposite X direction
            directionY = -captor.direction.y / length; // Opposite Y direction
        }
    }
    // Calculate position that is animalWidth + scaled NPC width + FOLLOW_DISTANCE units behind the user
    const FOLLOW_DISTANCE = 2; // Same constant as frontend
    const npcWidth = npcScale; // The scaled width of the NPC mesh
    const behindPosition = {
        x: captor.position.x + directionX * (animalWidth / 2 + npcWidth / 2 + FOLLOW_DISTANCE),
        y: captor.position.y + directionY * (animalWidth / 2 + npcWidth / 2 + FOLLOW_DISTANCE),
        z: 0.05, // Place in front of wave grid
    };
    return behindPosition;
}
// Handle reflection when thrown NPC hits captured NPC group (server-side version)
function handleCapturedNPCGroupReflection(room, thrownPathData, thrownNPCGroup, capturedNPCGroup, currentPathPosition, capturedGroupPosition) {
    // Calculate reflection direction (away from the captured group)
    ...
    const normalizedHash = getRandom({
        pathId: thrownPathData.id,
        npcGroupId: thrownNPCGroup.id,
        captorId: capturedNPCGroup.captorId,
        timestamp: thrownPathData.timestamp
    });
    ...
    const reflectionPathData = {
        ...
        velocity: thrownPathData.velocity, // Fast reflection speed
        timestamp: Date.now(),
        pathPhase: types_1.PathPhase.THROWN,
    };
    // Update paths in memory
    const paths = (0, paths_1.getpathsfromMemory)(room);
    paths.set(thrownNPCGroup.id, reflectionPathData);
    (0, paths_1.setPathsInMemory)(room, paths);
    // Broadcast reflection to clients
    (0, typed_socket_1.emitToRoom)(room, "path-update", { pathData: reflectionPathData });
    // Get the number of NPCs to emit based on thrown NPC group size
    const emittedNPCs = [];
    const emissionCount = Math.min(thrownNPCGroup.fileNames.length, capturedNPCGroup.fileNames.length);
    // Calculate impact direction (direction of the thrown NPC's movement/momentum)
    // This is the direction the thrown NPC was traveling when it hit
    const impactDirection = {
        x: thrownPathData.direction.x,
        y: thrownPathData.direction.y,
    };
    ...
    for (let i = 0; i < emissionCount; i++) {
        ...
        const emittedNPCGroup = new types_1.NPCGroup({
            id: (0, uuid_1.v4)(),
            ...
            position: capturedGroupPosition,
        });
    ...
    const restOfNPCsGroup = remainingNPCs.length > 0 ? new types_1.NPCGroup(Object.assign(Object.assign({}, capturedNPCGroup), { fileNames: remainingNPCs })) : null;
    if (emittedNPCs.length > 0) {
        // Emit from the collision point, not the captured group position
        // This prevents NPCs from being emitted from behind the player
        const emissionStartPosition = {
            x: currentPathPosition.x,
            y: currentPathPosition.y,
        };
        ...
        const paths = (0, paths_1.getpathsfromMemory)(room);
        const allNPCGroups = (0, npc_groups_1.getNPCGroupsfromMemory)(room);
        // First, create all the paths for emitted NPCs
        emittedNPCs.forEach((emittedNPC) => {
            const emissionPathData = {
                ...
                pathDuration: 3000, // Longer duration to prevent immediate recapture
                velocity: 3.0, // Moderate emission speed
                timestamp: Date.now(),
                pathPhase: types_1.PathPhase.FLEEING,
            };
            paths.set(emittedNPC.id, emissionPathData);
        });
        // Save paths to memory FIRST
        (0, paths_1.setPathsInMemory)(room, paths);
        // Then add NPC groups and broadcast
        emittedNPCs.forEach((emittedNPC) => {
            allNPCGroups.setByNpcGroupId(emittedNPC.id, emittedNPC);
            // Broadcast individual updates - paths first, then groups
            (0, typed_socket_1.emitToRoom)(room, "path-update", { pathData: paths.get(emittedNPC.id) });
            (0, typed_socket_1.emitToRoom)(room, "npc-group-update", { npcGroup: emittedNPC });
        });
        // Update or remove the original captured group
        if (restOfNPCsGroup) {
            allNPCGroups.setByNpcGroupId(restOfNPCsGroup.id, restOfNPCsGroup);
            (0, typed_socket_1.emitToRoom)(room, "npc-group-update", { npcGroup: restOfNPCsGroup });
        }
        else {
            // If no NPCs remain, delete the original group
            allNPCGroups.deleteByNpcGroupId(capturedNPCGroup.id);
            (0, typed_socket_1.emitToRoom)(room, "npc-group-update", { npcGroup: new types_1.NPCGroup(Object.assign(Object.assign({}, capturedNPCGroup), { fileNames: [] })) });
        }
        // Save final NPC groups state to memory
        (0, npc_groups_1.setNPCGroupsInMemory)(room, allNPCGroups);
    }
}
// Deterministic random function that produces consistent results across all clients (server-side version)
